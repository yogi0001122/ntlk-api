...
class scoreText(object):
    ...
    def sentenceMatchDB(self,keyword,paragraph):
        """
        Converts paragraph into list and calls scoreText and sortText functions,
        and returns the most matching sentences with the keywords.
        """
    ...
    def spellcheck(self,keyword):

        try:
            spell = SpellChecker()

            #questionstring = "please tell me my acccount balaance"

            questionstring = keyword

            question = [questionstring]

            combinestring = ""

            print (self.convert(question))
            # find those words that may be misspelled
            #misspelled = spell.unknown(convert(question))

            misspelled = self.convert(question)

            #if (misspelled):
             #   print(misspelled)
            #else :
             #   print("Not Found")
              #  combinestring = questionstring

            for word in misspelled:
                 # Get the one `most likely` answer
                 print(spell.correction(word))
                 combinestring =  combinestring+ " "+ spell.correction(word) 

                # Get a list of `likely` options
                #print(spell.candidates(word))

            print("Combined String:"+combinestring.strip(" "))
            return  (combinestring.strip(" "))
        except:
            print("Error Occured in in Spell check")              
